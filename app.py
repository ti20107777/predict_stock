...
st.title("AIで株価予測アプリ")
st.write('AIを使って、株価を予測しましょう')

image = Image.open('stock_predict.png')
st.image(image, use_column_width=True)

st.write('あくまでAIによる予測です（参考値）。こちらのアプリによる損害損出は一切補償しません。')

st.header("株価銘柄のティッカーシンボルを入力してください")
stock_name = st.text_input("例：AAPL, FB(大文字・小文字どちらでも可", "AAPL")
...
st.markdown(link)
st.write('ティッカーシンボルについては上のリンク（SBI証券）を参照')
